...
class EarCut
{
	...
	static public function triangulate(poly:Poly, ?holes:Array<Poly> = null):Array<Tri>
	{
		...
		if (holes != null && holes.length > 0)
		{
			...
			for (hole in holes)
			{
				for (f in PolyTools.toFloatArray(hole))
					data.push(f);
	...
	static function cureLocalIntersections(start:EarNode, triangles:Array<Int>, dim:Int):EarNode
	{
		...
		do
		{
			var a:EarNode = p.prev, b:EarNode = p.next.next;
	...
	{
		var queue:Array<EarNode> = [], i, len, start, end, list;
	...
	{
		var p = outerNode, hx = hole.x, hy = hole.y, qx = Math.NEGATIVE_INFINITY, m = null;
		...
		var stop = m, mx = m.x, my = m.y, tanMin = Math.POSITIVE_INFINITY, tan;
	...
	{
		var i, p:EarNode = null, q:EarNode = null, e:EarNode = null, tail:EarNode = null, numMerges, pSize, qSize, inSize = 1;
	...
	static function pointInTriangle(ax:Float, ay:Float, bx:Float, by:Float, cx:Float, cy:Float, px:Float, py:Float):Bool
	{
		return (cx
			- px) * (ay - py)
			- (ax - px) * (cy - py) >= 0
				&& (ax - px) * (by - py) - (bx - px) * (ay - py) >= 0 && (bx - px) * (cy - py) - (cx - px) * (by - py) >= 0;
	}
	...
	{
		var a2 = new EarNode(a.i, a.x, a.y), b2 = new EarNode(b.i, b.x, b.y), an = a.next, bp = b.prev;
	...
	static public function addTriangle(poly:Poly, t:Tri):Poly
	{
		...
		for (i in 0...poly.length)
		{
			...
			if (i == firstP) newPoints.push(t[tipT]);
		}
