...
class HomogCoord
{
	...
	public function add(p:HomogCoord):HomogCoord
	{
		x += p.x;
		y += p.y;
		return this;
	...
	public function sub(p:HomogCoord):HomogCoord
	{
		x -= p.x;
		y -= p.y;
		return this;
	...
	public function neg():HomogCoord
	{
		w = -w;
		x = -x;
		y = -y;
	...
	public function mul(m:Float):HomogCoord
	{
		w *= m;
		x *= m;
		y *= m;
	...
	public function div(m:Float):HomogCoord
	{
		w /= m;
		x /= m;
		y /= m;
	...
	public function perp():HomogCoord
	{
		var tmp:Float = -y;
		y = x;
		x = tmp;
	...
	public function dotPoint(p:HxPoint)
	{
		return w + x * p.x + y * p.y;
	}

	public function dot(p:HomogCoord)
	{
		return w * p.w + x * p.x + y * p.y;
	}

	public function perpdot(p:HomogCoord)
	{
		return x * p.y - y * p.x;
	}

	public function dotperp(p:HomogCoord)
	{
		return -x * p.y + y * p.x;
	}

	public function equals(p:HomogCoord)
	{
		return (p.w * x == w * p.x) && (p.w * y == w * p.y);
	}

	public function left(p:HxPoint)
	{
		return dotPoint(p) > 0;
	}

	public function right(p:HxPoint)
	{
		return dotPoint(p) < 0;
	}
	...
	public function toPoint():HxPoint
	{
		return new HxPoint(x / w, y / w);
	}
