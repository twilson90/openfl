...
class PolyTools
{
	...
	**/
	...
	inline static public function distance(v:HxPoint, w:HxPoint)
	{
		return Math.sqrt(distanceSquared(v, w));
	}

	/** Perpendicular distance from `p` to line segment `v`-`w`. */
	inline static public function distanceToSegment(p:HxPoint, v:HxPoint, w:HxPoint)
	{
		return Math.sqrt(distanceToSegmentSquared(p, v, w));
	}

	/** Squared distance from `v` to `w`. */
	inline static public function distanceSquared(v:HxPoint, w:HxPoint):Float
	{
	...
	static public function getCentroid(poly:Poly):HxPoint
	{
		...
		{ // avoid division by zero and return an empty Point
			c = HxPoint.EMPTY;
		}
		return c;
	}
	...
	@:noUsing static public function inflateLine(start:HxPoint, end:HxPoint, thickness:Float):Poly
	{
		...
		return [
			new HxPoint(start.x - ny, start.y + nx),   new HxPoint(end.x - ny, end.y + nx),
	...
	{
		#if (js && !jsprime)
		var dotPath = (as != null ? as : enumClass.getName()).split("."); untyped
		{
